...
export default function VideoGallery({ createMailLink }) {
    ...
    useEffect(() => {
        async function fetchVideos() {
            try {
                const response = await fetch("https://ergopro-ecloud.equeo.de/rest/v1/videos?tags=ergo,ergo%20pro&limit=100&offset=0", {
                    headers: {
    ...
    return (
        <div className="flex flex-1">
                    ...
                    {videos.length !== 0 && (
                        <table className="table-auto text-xs sm:text-sm text-left rtl:text-right text-body w-full">
                            ...
                            <tr>
                                ...
                                <th className="px-6 py-3">Upload Datum</th>
                                    ...
                                    <tr key={key} className="align-middle">
                                        <td className="px-4 sm:px-6 py-2 sm:py-4">
                                            <div className="flex items-center gap-3 min-w-88">
                                                <img className="w-32 flex-none" src={createThumbnailLink(key)} alt="" />
                                                <p className="min-w-0 flex-1 wrap-anywhere line-clamp-2">
                                                    {title}
                                                </p>
                                            </div>
                                        </td>
                                        <td className="px-4 sm:px-6 py-2 sm:py-4 whitespace-nowrap">{status}</td>
                                        <td className="px-4 sm:px-6 py-2 sm:py-4 whitespace-nowrap">{duration}</td>
            ...
            <div className="w-full p-2 block sm:hidden">
                {videos.length === 0 ? (
                    <div
                        className="w-full rounded-xl border border-dashed border-gray-300 p-6 text-center text-sm text-gray-500">
                        Noch keine Videos hochgeladen.
                    </div>
                ) : (
                    <div>
                        {videos.map((video) => (
                            < VideoCard video={video} convertDuration={convertDuration} formatDateTime={formatDateTime} createThumbnailLink={createThumbnailLink}/>
                        ))}
                    </div>
                )}
            </div>
